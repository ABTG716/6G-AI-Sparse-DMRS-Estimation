...
Self-Supervised Learning (label-free, Alt.1 principle):
  ...
  └─ Reconstruction loss vs. noisy LS at masked positions — no ideal H needed
...
import os, math, time, logging, argparse
...
class Config:
    ...
    # ── MLP-Mixer model (tuned for ~14 MFLOPs budget) ────────────────────
    d_model:         int   = 32     # hidden dimension
    n_layers:        int   = 2      # number of Mixer blocks
    token_mix_dim:   int   = 64     # hidden dim for token-mixing MLP (spatial)
    channel_mix_dim: int   = 64     # hidden dim for channel-mixing MLP (feature)
    dropout:         float = 0.05   # less regularisation for smaller model
    drop_path:       float = 0.05   # stochastic depth max rate
    snr_cond:        bool  = True   # FiLM SNR conditioning

    # ── self-supervised masking ────────────────────────────────────────────
    mask_ratio: float = 0.25        # fraction of DMRS positions masked per sample

    # ── training ───────────────────────────────────────────────────────────
    batch_size:   int   = 64
    lr:           float = 3e-4
    weight_decay: float = 1e-4
    epochs:       int   = 80        # more epochs for smaller model to converge
    warmup_pct:   float = 0.06      # fraction of steps for LR warm-up
    grad_clip:    float = 1.0
    seed:         int   = 42

    # ── data split ─────────────────────────────────────────────────────────
    train_frac: float = 0.80        # 81 920 samples
    val_frac:   float = 0.10        # 10 240 samples  (test = remaining 10 240)
...
class Alt1Dataset(Dataset):
    ...
    def __init__(self, h5_path: str, indices: np.ndarray, cfg: Config):
        ...
        with h5py.File(h5_path, "r") as f:
            dmrs_mask = f["dmrs_mask"][:]               # [612, 14]  bool
            self._snr = f["snr_db"][:].astype(np.float32)

        sc_idx, sym_idx = np.where(dmrs_mask)
    ...
    def __getitem__(self, idx: int):
        ...
        ls_re = self._file["input_ls_real"][:, :, :, s]  # [612, 14, 2]
        ls_im = self._file["input_ls_imag"][:, :, :, s]

        # extract DMRS positions ───────────────────────────────────────────
        re = ls_re[self._sc, self._sym, :]               # [N_dmrs, 2]
...
def ssl_loss(pred: torch.Tensor,
             target: torch.Tensor,
             mask: torch.Tensor) -> torch.Tensor:
    """
    MSE on masked tokens only (pseudo-label = noisy LS estimate).
...
def run_epoch(model, loader, cfg, device,
    ...
    total_ssl, total_full, n = 0.0, 0.0, 0
    ...
    for batch in loader:
        ...
        # ── forward (AMP always wraps correctly) ──────────────────────────
        with torch.cuda.amp.autocast(enabled=use_amp):
            pred = model(x, pos, snr, mask)
            loss = ssl_loss(pred, x, mask)
...
def evaluate_nmse(model: Alt1Mixer, h5_path: str,
    ...
    with h5py.File(h5_path, "r") as f:
        h_re_all = f[f"{label}_real"][:]                          # [408, 2, N]
    ...
    for i, batch in enumerate(loader):
        ...
        h_re = h_re_all[:, :, sidx].transpose(2, 0, 1)           # [B, N, 2]
        h_im = h_im_all[:, :, sidx].transpose(2, 0, 1)
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f"{cfg.log_dir}/training.log"),
        ],
    )
    log = logging.getLogger(__name__)
    log.info("═" * 66)
    log.info("  Alt.1 MLP-Mixer — Self-Supervised DMRS Denoiser")
    log.info("═" * 66)
    ...
    log.info(f"  Drop path   : {cfg.drop_path}")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        tr = run_epoch(model, train_dl, cfg, device, opt, sched, scaler)
        va = run_epoch(model, val_dl,   cfg, device)
        ...
        if va["ssl"] < best_val:
            ...
            torch.save(
                {"epoch": epoch, "state": model.state_dict(),
                 "opt":   opt.state_dict(), "val_ssl": best_val,
                 "cfg":   cfg},
                f"{cfg.ckpt_dir}/best.pt",
            )
            log.info(f"  ↳ ✓ Best checkpoint saved  (val_ssl={best_val:.4f})")

    torch.save(model.state_dict(), f"{cfg.ckpt_dir}/final.pt")
    np.save(f"{cfg.log_dir}/history.npy", history)
    log.info("Training complete.")

    # ── offline NMSE benchmarks ───────────────────────────────────────────
    log.info("Running NMSE evaluation on test set …")
    ckpt = torch.load(f"{cfg.ckpt_dir}/best.pt", map_location=device,
                      weights_only=False)
    model.load_state_dict(ckpt["state"])

    nmse_ideal = evaluate_nmse(model, cfg.h5_path, splits["test"],
                               cfg, device, label="alt1_ideal")
    nmse_pract = evaluate_nmse(model, cfg.h5_path, splits["test"],
