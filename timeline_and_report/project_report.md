...
## 1.1 `DLSCH_PDSCH_script.m` — The Foundation
...
## 1.2 `continuous_dataset_generator_Final.m` — Continuous Waveform
...
## 1.4 `alt1/` Directory — Samsung Alt.1 Self-Supervised Denoiser

Based on Samsung's proposal at the **3GPP RAN1 #119bis meeting in Prague (October 2025)**, document R1-2500xxx.

### 1.4.1 `xyz.py` — Version 1 (Original Implementation)
...
### 1.4.2 `alt1_transformer.py` — Version 2 (Fixed Implementation)

This is the **corrected version** after diagnosing v1's failures. Despite the filename saying "transformer", it is still an **MLP-Mixer**.

#### What Changed (v1 → v2)

| Aspect | v1 (`xyz.py`) | v2 (`alt1_transformer.py`) |
|--------|---------------|---------------------------|
| d_model | 32 | **48** |
| Layers | 2 | **3** |
...
Parses `log.txt` and generates a **5-panel dashboard**:
...
### 1.4.5 `log.txt` — Training Log (v2 Results)
...
## 1.5 Training Dataset (`dataset_sparse_fd.h5`) — Hosted Externally

> [!IMPORTANT]
> The Alt.1 model was **NOT** trained on datasets produced by the MATLAB scripts in this folder. It was trained on a separate, purpose-built 22 GB HDF5 dataset hosted on OneDrive due to its size.
...
> [!NOTE]
> The MATLAB scripts in this folder (`DLSCH_PDSCH_script.m`, `continuous_dataset_generator_Final.m`, `Multiple_UE_v1.m`) document the **evolution of the simulation approach** and were used for prototyping. The final training dataset was generated by a separate, production-scale MATLAB script (not in this folder) that produces the HDF5 format expected by the Python code.
...
#### Phase 2a: Basic 5G NR Simulation (`DLSCH_PDSCH_script.m`)
...
#### Phase 2b: Continuous Waveform (`continuous_dataset_generator_Final.m`)
...
#### Phase 2c: Multi-UE Scaling (`Multiple_UE_v1.m`)
...
#### Experiment 3c: Version 2 (`alt1_transformer.py`) — SUCCESS
...
```mermaid
graph TD
    A["DLSCH_PDSCH_script.m<br/>Basic per-slot simulation<br/>20 slots, SVD precoding"] --> B["continuous_dataset_generator_Final.m<br/>Continuous waveform<br/>400 slots, random precoding"]
    B --> C["Multiple_UE_v1.m<br/>300 independent UEs<br/>120,000 total snapshots"]
    C -->|"Prototyping evolution"| X["Production dataset script<br/>(not in folder)"]
    X -->|"dataset_sparse_fd.h5<br/>102,400 samples, 22 GB<br/>(OneDrive)"| D["alt1/xyz.py (v1)<br/>First Alt.1 attempt<br/>FAILED: +0.46 dB NMSE"]
    D -->|"Diagnosis & Fix"| E["alt1/alt1_transformer.py (v2)<br/>Fixed Alt.1 implementation<br/>SUCCESS: −15.91 dB NMSE"]
    E --- F["alt1/plot_metrics.py<br/>Visualisation"]
    E --- G["alt1/count.py<br/>FLOPs counter"]
    E --- H["alt1/log.txt<br/>Training log (120 epochs)"]
```
...
5. The **MLP-Mixer backbone** faithfully implements Samsung's Alt.1 and achieves strong results (−15.91 dB NMSE) with only 273K parameters
