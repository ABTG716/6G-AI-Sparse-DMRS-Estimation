...
```mermaid
graph TD
    A[01: Basic NR Sim] --> B[02: Continuous Waveform]
    B --> C[03: Multi-UE Scaling]
    C -->|Prototyping| X[Production Dataset Script]
    X -->|dataset_sparse_fd.h5| D[Alt.1 v1 — FAILED]
    D -->|Diagnosis & Fix| E[Alt.1 v2 — SUCCESS]
    E --- F[Plot Metrics]
    E --- G[Count FLOPs]
    E --- H[Training Log]
```
...
| Node | File | Details |
|------|------|---------|
| **01** | `dataset_gen/01_basic_nr_simulation.m` | Basic per-slot simulation — 20 slots, SVD precoding |
| **02** | `dataset_gen/02_continuous_waveform.m` | Continuous waveform — 400 slots, random precoding |
| **03** | `dataset_gen/03_multi_ue_scaling.m` | 300 independent UEs — 120,000 total snapshots |
| **Production** | `dataset_gen/final_dataset_script_for_denoiser/generate_ai_dataset.m` | Generates `dataset_sparse_fd.h5` (102,400 samples, 22 GB) — [OneDrive](https://1drv.ms/f/c/630f9b47e52a7119/IgD1YY3KvhmTQbQGa6NlAH59AUw5l8LlGeAF9gJUGRve2zQ?e=HQAhgg) |
| **v1** | `denoiser_transformer/alt1_mixer_v1.py` | First attempt — FAILED: +0.46 dB NMSE |
| **v2** | `denoiser_transformer/alt1_mixer_v2.py` | Fixed implementation — SUCCESS: −15.91 dB NMSE |
| **Plot** | `denoiser_transformer/plot_metrics.py` | Training visualisation dashboard |
| **FLOPs** | `denoiser_transformer/count_flops.py` | Model FLOPs counter |
| **Log** | `results/training_log.txt` | Training log (120 epochs) |
