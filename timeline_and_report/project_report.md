...
> [!NOTE]
> The MATLAB scripts in `dataset_gen/` (`01_basic_nr_simulation.m`, `02_continuous_waveform.m`, `03_multi_ue_scaling.m`) document the **evolution of the simulation approach** and were used for prototyping. The final training dataset was generated by a separate, production-scale MATLAB script (not in this repo) that produces the HDF5 format expected by the Python code.
...
graph TD
    ...
    C -->|"Prototyping evolution"| X["Production dataset script<br/>(not in repo)"]
